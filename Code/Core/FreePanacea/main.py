...
PANACEA_PATH : str = "Core/Sounds/mm_xp_chime.wav"
VERSION_MAJOR : str = '1'
VERSION_MINOR : str = '3'
...
def Main() -> None:
    ...
    Banner(Fore.YELLOW, "PANACEA", f"The thing, that'll mute the \"exp gained\" sound effect!\nVersion: {VERSION_MAJOR}.{VERSION_MINOR} -- free account version by Wormyy")
    
    action : str = input("1. Fix the sound\n2. Bring the default tf2 \"exp gained\" sound back\n3. Leave\n\n>>> ")
...
def SoundRemover() -> None:
    ...
    try:
        ...
        if path.exists(path.abspath(tf_path+EXTRA_PATH+"mm_xp_chime.wav")):
            ...
            rmtree(tf_path+'/tf/custom/ThePanaceaProject')
